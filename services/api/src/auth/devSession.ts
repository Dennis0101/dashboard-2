...
export type DevSession = {
  provider: string;
  subject: string;
  userId: string; // stable internal uuid
};

function bytesToUuidV4(bytes: Buffer): string {
...
export function parseDevSessionToken(token: string): DevSession | null {
  // format: dev-session:<provider>:<subject>
  const parts = token.split(":");
  if (parts.length < 3) return null;
  if (parts[0] !== "dev-session") return null;
